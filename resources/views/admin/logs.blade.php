<x-app-layout>
	<x-slot name="title">Catatan Aktivitas</x-slot>
	...
	<x-card>
		<x-slot name="title">Semua Catatan Aktivitas</x-slot>
		<x-slot name="option">
			<form action="{{ route('admin.logs.delete') }}" method="post">
				@csrf
				<button type="submit" class="btn btn-danger">Hapus 7 hari yang lalu</button>
			</form>
		</x-slot>
